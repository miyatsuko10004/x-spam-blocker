...
async function extractTargetsFromSearch(page, keyword, currentUser) {
    ...
    for (const target of targets) {
        ...
        if (target.username.includes('/'))
            continue;
        if (excludedPaths.some(p => usernameLower === p))
            continue;
        if (currentUser && usernameLower === currentUser.toLowerCase())
            continue;
        if (!usernameRegex.test(target.username))
            continue;
...
async function run() {
    ...
    try {
        ...
        for (let iter = 1; iter <= maxIterations; iter++) {
            ...
            console.log(`=== Starting search scan for keyword: "${config_js_1.config.targetSearchQuery}" ===`);
            const targetUsers = await extractTargetsFromSearch(page, config_js_1.config.targetSearchQuery, currentUser);
            console.log(`Found ${targetUsers.length} potential targets for keyword "${config_js_1.config.targetSearchQuery}".`);
            let skipCount = 0;
            for (const target of targetUsers) {
                ...
                // スパム判定 (定型文マッチ)
                const isSpam = (0, spamDetector_js_1.isSpamTweet)(tweetText);
                if (isSpam) {
